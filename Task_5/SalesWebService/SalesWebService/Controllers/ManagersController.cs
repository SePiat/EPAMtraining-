    ...
    public class ManagersController : Controller
    {
        ...
        public async Task<ActionResult> Edit(Manager model)
        {
            using (var context = new ApplicationDbContext())
            {
                ...
                {
                    Log.Error($"ManagersController.Edit: Ошибка при попытке сохранения нового менеджера: {DateTime.Now}" +
                           $"{Environment.NewLine}{e}{Environment.NewLine}");
                    throw new Exception($"Ошибка при попытке сохранения нового менеджера: {e}");
                }
            }
            return View("Index");
        }
        ...
        [Authorize]
        public ActionResult SearchByCountBuyers(string SearchCountBuyers)
        {
            if (SearchCountBuyers != null)
            {
                bool isIntSearchCountBuyers = int.TryParse(SearchCountBuyers, out int count);
